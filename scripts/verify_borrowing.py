...
out=Path('reports/v2/borrowing');out.mkdir(parents=True,exist_ok=True)
for name,expected in [('before',1),('after',0),('unknown',3)]:
    target=out/(name+'.json')
    r=subprocess.run([args.binary,'analyze','examples/v2/borrowing/'+name,'--entry','case::run','--format','json','--output',str(target)],capture_output=True,text=True)
    ...
    if name=='before':
        ...
        assert len(errors)==1 and errors[0]['key']['rule']=='BORROW001'
...
print('BORROW001 policy before/repair/unknown:1/0/3; compare resolved/became_unverified.')
